...
def analizar_texto_completo(texto):
    ...
    print("REALIZANDO ANÁLISIS COMPLETO DEL TEXTO")
    print("=" * 40)
    ...
    sentimiento_info = analizar_sentimiento_basico(texto)
    print(f"Sentimiento general: {sentimiento_info['sentimiento']}")
    print(f"Palabras positivas encontradas: {sentimiento_info['palabras_positivas_encontradas']}")
    print(f"Palabras negativas encontradas: {sentimiento_info['palabras_negativas_encontradas']}")
    ...
    # Palabras repetidas
    print("\n--- Palabras Repetidas (>= 4 letras) ---")
    palabras_repetidas = encontrar_palabras_repetidas(texto)
    if palabras_repetidas:
        # Ordenar por frecuencia (mayor a menor)
        repetidas_ordenadas = sorted(palabras_repetidas.items(), key=lambda x: x[1], reverse=True)
        for palabra, frecuencia in repetidas_ordenadas:
            print(f" - '{palabra}': {frecuencia} veces")
    else:
        print("No se encontraron palabras repetidas (con longitud >= 4).")

# Ejemplo de uso
ejemplo_texto = """
Este es un texto de ejemplo. Contiene varias oraciones
y dos párrafos.

Aquí hay otro párrafo con algunas palabras repetidas,
como texto y palabras. También podría haber un email
de ejemplo: prueba@dominio.com y un número de teléfono
en formato (123) 456-7890 o 987-654-3210. Este texto
es bastante bueno y genial, pero no es perfecto.
"""

analizar_texto_completo(ejemplo_texto)