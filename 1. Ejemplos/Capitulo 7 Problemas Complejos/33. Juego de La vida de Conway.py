...
def mostrar_tablero(tablero, generacion, mostrar_coordenadas=False):
    ...
    if mostrar_coordenadas:
        print(" " * 3 + " ".join([str(i % 10) for i in range(len(tablero[0]))])) # Coordenadas de columna
        print(" " * 2 + "-" * (len(tablero[0]) * 2 + 1)) # Separador

    for i, fila in enumerate(tablero):
        linea = ""
        if mostrar_coordenadas:
            linea += f"{i % 10}: " # Coordenada de fila
        for celula in fila:
            linea += "■ " if celula == 1 else "□ " # ■ para vivo, □ para muerto
        print(linea)